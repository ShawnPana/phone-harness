...
def tap(text):
    ...
    node = nodes[0]
    phone.send('input.tap', x=node['x'] + node['w']/2, y=node['y'] + node['h']/2)
...
try:
    phone = measured('attach_existing_session', lambda: CloudPhone(session_id=args.session))
    ...
    if phone is not None and args.release:
        try:
            report['release_receipt'] = measured('release', phone.release)
            def wait_cleanup():
                deadline = time.monotonic() + 120
                while True:
                    sessions = phone._http('GET', '/sessions')
