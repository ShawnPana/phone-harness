...
class BackgroundDeliveryTests(unittest.TestCase):
    ...
    def test_keyboard_uses_device_hid_without_fronting_mirroring(self):
        with mock.patch.object(background.device_hid, "press") as press, \
                mock.patch.object(background.device_hid, "type_text") as type_text, \
                mock.patch.object(background, "activate") as activate:
            background.press("return")
            background.type_text("hello", delay=0.02)

        press.assert_called_once_with("return")
        type_text.assert_called_once_with("hello", delay=0.02, keystrokes=False)
